...
## as well as the associated figures (all panels of Fig. 2), with the stats done by complete pooling at the tube level:
## measurements from different times are averaged for each patch.
...
## Subsetting the data ##################################################################################################
  ...
  # agregating
  agg_formula_1 = cbind(bioarea, local_simpson, BetaDiv) ~ 
    tube_id + landscape + extinction + n_extinctions + correlation
  post_ext_data_aggregate = aggregate(agg_formula_1, data = post_ext_data, FUN = mean, na.action = na.omit)
  post_ext_landscape_data_aggregate = aggregate(agg_formula_1, data = post_ext_landscape_data, FUN = mean, na.action = na.omit)
  ...
  # aggregating by tube id (taking the mean of each time)
  agg_formula_2 = cbind(bioarea, local_simpson, BetaDiv) ~ 
    tube_id + landscape + extinction + n_extinctions + correlation +
    correlation_label + n_extinctions_label + n_extinctions_factor + extinction_label
  fig_data_agregate = aggregate(agg_formula_2, data = fig_data, FUN = mean, na.action = na.omit)
  
  # landscape-scale data for the analysis of beta diversity
  ...
  # aggregating by landscape ID
  agg_formula_3 = BetaDiv ~ 
    tube_id + landscape + extinction + n_extinctions + correlation +
    correlation_label + n_extinctions_label + n_extinctions_factor + extinction_label
  
  fig_data_landscape_aggregate = aggregate(agg_formula_3, data = fig_data_landscape, FUN = mean, na.action = na.omit)

## Alpha-diversity full model ############################################################################################
  ## looking for best normalization method
  normalize.choice.alpha = bestNormalize(post_ext_data_aggregate$bioarea)$norm_stats
  for (k in 1:99){
    normalize.choice.alpha = rbind(normalize.choice.alpha, bestNormalize(post_ext_data_aggregate$bioarea)$norm_stats)
  }
  summary(normalize.choice.alpha)
  
  ## normalizing data
  norm_simpson = orderNorm(post_ext_data_aggregate$local_simpson)
  post_ext_data_aggregate$norm_simpson = norm_simpson$x.t
  
  ## mixed model declaration
  mod_simpson = lmer(norm_simpson ~ as.factor(n_extinctions)*as.factor(correlation) + (1 | landscape),
                     data = post_ext_data_aggregate,
...
## Beta-diversity full model #################################################################################################
  ...
  beta_data = post_ext_landscape_data_aggregate
  bestNormalize(beta_data$BetaDiv, loo = T) ## no transformation necessary
  
  norm_beta = orderNorm(beta_data$BetaDiv)
  beta_data$norm_beta = norm_beta$x.t
  
  
  mod_beta = lm(norm_beta ~ as.factor(n_extinctions)*as.factor(correlation),
                  data = beta_data)
  
## Bioarea full model #######################################################################################################
  ## same process as above
  normalize.choice.bioarea = bestNormalize(post_ext_data_aggregate$bioarea)$norm_stats
  for(k in 1:99){
    normalize.choice.bioarea = rbind(normalize.choice.bioarea, bestNormalize(post_ext_data_aggregate$bioarea)$norm_stats)
  }
  summary(normalize.choice.bioarea)
    
  norm_bioarea = orderNorm(post_ext_data_aggregate$bioarea)
  post_ext_data_aggregate$norm_bioarea = norm_bioarea$x.t
  
  mod_bioarea = lmer(norm_bioarea ~ as.factor(n_extinctions)*as.factor(correlation) + (1 | landscape),
                     data = post_ext_data_aggregate,
                     control = lmerControl(optimizer = "bobyqa"),
                     REML = F)
...
## predictions alpha ############################################################################################################
  ...
  mod_simpson_list = get.models(mod_simpson_dredge, subset = T)
...
## predict beta ###########################################################################################
  ...
  mod_beta_list = get.models(mod_beta_dredge, subset = T)
  ...
  for (k in 1:5){
    mod_beta_coefficients_density[[k]] = mvrnorm(n = as.integer(n_pred * mod_beta_weights[k]),
                                                 mod_beta_coefficients[[k]], mod_beta_vcov[[k]])
  }
  
  predict_beta = function(coeff, data, model){
    ext = data$n_extinctions == 8
    cor = data$correlation
    if (model == 5){ # bioarea ~ cor
      return(coeff[1] + cor*coeff[2])
    }
    if (model == 2){ # bioarea ~ cor + ext
      return(coeff[1] + cor*coeff[2] + ext*coeff[3])
    }
    if (model == 4){ # full model
      return(coeff[1] + cor*coeff[2] + ext*coeff[3] + (ext*cor)*coeff[4])
    }
    if (model == 3){ # intercept model
      dummy = rep(1, nrow(data))
      return(dummy*coeff[1])
    }
    if (model == 1){ # bioarea ~ ext
      return(coeff[1] + ext*coeff[2])
    }
  }
...
## predict bioarea ########################################################################
  ...
  mod_bioarea_list = get.models(mod_bioarea_dredge, subset = TRUE)
  ...
  predict_bioarea = function(coeff, data, model){
    ext = data$n_extinctions == 8
    cor = data$correlation
    if (model == 2){ # bioarea ~ cor
      return(coeff[1] + cor*coeff[2])
    }
    if (model == 4){ # bioarea ~ cor + ext
      return(coeff[1] + cor*coeff[2] + ext*coeff[3])
    }
    if (model == 5){ # full model
      return(coeff[1] + cor*coeff[2] + ext*coeff[3] + (ext*cor)*coeff[4])
    }
...
## Making the predictions for all variables ####################################################################################
  ...
  predictions_table$norm_beta = NA
  ...
  for (model in 1:5){
    ...
    for (i in 1:nrow(coef_list)){
      ...
      predictions_table$norm_beta[n:(n+3)] = beta
      n = n +4
    }
  }
  ...
  predictions_table$beta = predict(norm_beta, predictions_table$norm_beta, inverse = T)
...
## plot predictions ##############################################################################################
  ...
  names(colors) = levels(post_ext_data_aggregate$correlation_label)
  colScale = scale_colour_manual(name = "Treatment",values = colors)
  fillScale = scale_fill_manual(name = "Treatment",values = colors)
  

  g = ggplot(data = fig_data_agregate, mapping = aes(x = as.factor(n_extinctions), fill = correlation_label,
                                          color = correlation_label, group = correlation_label))

  h = ggplot(data = fig_data_landscape_aggregate, mapping = aes(x = as.factor(n_extinctions), fill = correlation_label,
                                                    color = correlation_label, group = correlation_label))
  ...
  panel_d = h +
    geom_jitter(aes(y=BetaDiv), position = position_jitterdodge(jitter.width = 0.1), alpha = alpha_points, size = 0.5) +
    colScale + 
  ...
  panel_b = ggplot(data = tube_table[tube_table$extinctions == T,], mapping = aes(x = as.factor(n_extinctions), fill = correlation_label,
                                                                                  color = correlation_label, group = correlation_label)) +
    geom_jitter(aes(y=return_time), position = position_jitterdodge(jitter.width = 0.1), alpha = alpha_points, size = 0.5) +
    colScale + 
  ...
  fig2 = ggarrange(panel_a, panel_b, panel_c, panel_d,
            labels = "auto",
            common.legend = TRUE, legend = "bottom")
  
  ggsave("./fig2_mean.pdf",
         fig2,
         width = 173,
         height = 123,
         unit = "mm")
