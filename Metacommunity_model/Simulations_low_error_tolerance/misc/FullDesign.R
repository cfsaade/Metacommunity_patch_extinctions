...
landscape.tab$extinctions = rep(c(0, 2:15), each = 2*n_rep)                                 # Number of patches going extinct in
landscape.tab$replicate = rep(1:n_rep, 30)                                                 # each lanscape

landscape.tab$cor = rep(rep(c(F,T), each = n_rep), 15)                     # Color of the extinctions :
                                                                                # T = autocorrelation
...
tube.tab$position = rep(1:16, 15)     # la position dans le patch, 1 en haut a gauche puis 
...
write.csv(tube.tab, file = "FullTubeTable.csv")
write.csv(landscape.tab, file = "FullLandscapeTable.csv")
